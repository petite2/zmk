...
static inline void setup_alert(const struct device *dev, bool enable) {
    ...
    const struct pim447_config *cfg = (const struct pim447_config *)dev->config;
...
int pim447_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
...
static void pim447_gpio_callback(const struct device *dev, struct gpio_callback *cb,
                                 uint32_t pins) {
    struct pim447_data *data = CONTAINER_OF(cb, struct pim447_data, alert_cb);
...
int pim447_init_interrupt(const struct device *dev) {
    ...
    if (gpio == NULL) {
        LOG_DBG("Failed to get pointer to %s device!", cfg->alert_gpio_name);
        return -EINVAL;
    }
    data->alert_gpio = gpio;

    rc = gpio_pin_configure(gpio, cfg->alert_pin, GPIO_INPUT | cfg->alert_flags);
    if (rc != 0) {
        LOG_DBG("Failed to configure alert pin %u!", cfg->alert_pin);
        return -EIO;
    }

    gpio_init_callback(&data->alert_cb, pim447_gpio_callback, BIT(cfg->alert_pin));
    ...
    if (i2c_write_read(i2c, address, int_tx_rx_buf, sizeof(int_tx_rx_buf) - 1, int_tx_rx_buf + 1,
                       sizeof(int_tx_rx_buf) - 1) < 0) {
    ...
    k_thread_create(&data->thread, data->thread_stack, CONFIG_PIM447_THREAD_STACK_SIZE,
                    (k_thread_entry_t)pim447_thread, dev, 0, NULL,
                    K_PRIO_COOP(CONFIG_PIM447_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_PIM447_TRIGGER_GLOBAL_THREAD)
