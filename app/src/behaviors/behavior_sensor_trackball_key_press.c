...
enum orientation { EAST, NORTH, SOUTH, WEST };
...
static int on_sensor_binding_triggered(struct zmk_behavior_binding *binding,
    ...
    if ((click.val1 == 0b01) || (click.val1 == 0b10)) {
        ...
        rc = ZMK_EVENT_RAISE(zmk_mouse_button_state_changed_from_encoded(
            binding->param2, (click.val1 == 0b01), timestamp));
        // k_msleep(5);
    } else {
        ...
        rc = ZMK_EVENT_RAISE(zmk_trackball_state_changed_from_encoded(binding->param1, x_value.val1,
                                                                      y_value.val1, timestamp));
...
#define KP_INST(n)                                                                                 \
    static struct behavior_sensor_trackball_key_press_config                                       \
        behavior_sensor_trackball_key_press_config_##n = {                                         \
            .scan_interval_ms = DT_INST_PROP(n, scan_interval_ms),                                 \
            .orientation = DT_ENUM_IDX(DT_DRV_INST(n), orientation),                               \
    };                                                                                             \
    DEVICE_DT_INST_DEFINE(n, behavior_sensor_trackball_key_press_init, NULL, NULL,                 \
                          &behavior_sensor_trackball_key_press_config_##n, APPLICATION,            \
